...
export function holeOffsetsFromPoint(box, point){
const l=box[0],t=box[1],sw=box[2],sh=box[3];
const scale=Math.min(200/sh,220/sw),w=sw*scale,h=sh*scale;
const u=(point.x-l)/sw,v=(point.y-t)/sh;
return {holeX:Math.round(u*w-w/2),holeY:Math.round(15+v*h)};
}
